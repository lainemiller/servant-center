...
export class HealthTrackerComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,private service:HealthTrackerService) {}
  ...
  showFilledForm()
  {
    for(let i=0;i<this.healthTrackerDetails['result'].length;i++){
      var trackingSubject=this.healthTrackerDetails['result'][i]['tracking_subject'];
  ...
  getHealthTrackerByVeteranId(){
    var veteranId:number=1;
    let resp=this.service.getHealthTrackerByVeteranId(veteranId);
    resp.subscribe((data)=>{
  ...
  onSubmit() {
    ...
    this.service.addHealthTrackerByVeteranID(1,this.healthTrackerFormDetails).subscribe((response:any)=>{
